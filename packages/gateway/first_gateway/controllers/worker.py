    ...
    async def supervise(self, shutdown: asyncio.Event) -> None:
        ...
        while not shutdown.is_set():
            ...
            try:
                ...
                if shutdown.is_set():
                    logger.info("worker %s cancelled in shutdown; stopping", self.name)
                    raise
                logger.warning(
                    "worker %s task cancelled without shutdown; restarting", self.name
                )
                backoff = self._restart_backoff
            except Exception:
                logger.exception(
                    "worker %s crashed; restarting in %.1fs", self.name, backoff
                )
                try:
