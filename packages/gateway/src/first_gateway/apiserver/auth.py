...
def get_globus_client() -> globus_sdk.ConfidentialAppAuthClient:
    ...
    return globus_sdk.ConfidentialAppAuthClient(
        cfg.app_id, cfg.app_secret.get_secret_value()
    )
...
async def _perform_token_introspection(bearer_token: str) -> TokenIntrospectionResult:
    """
    ...
    if len(cfg.policies) > 0:
        introspect_body["authentication_policies"] = cfg.policies_str
    ...
    try:
        dependent_tokens = await asyncio.to_thread(
            client.oauth2_get_dependent_tokens, bearer_token
        )
...
def check_globus_policies(
    ...
    cfg = Settings.load().globus
    if len(introspection["policy_evaluations"]) != len(cfg.policies):
        return (
            False,
    ...
    for policies in introspection["policy_evaluations"].values():
        if policies.get("evaluation", False) == False:
            ...
            error_message += f"{cfg.authorized_idp_domains_str}."
...
def check_globus_groups(user_groups: list[str]) -> tuple[bool, str]:
    ...
    if len(set(user_groups).intersection(cfg.user_groups)) > 0:
        return True, ""
...
def check_session_info(
    ...
    error_message = ""
    error_message += f"Error: Permission denied. Must authenticate with {cfg.authorized_idp_domains_str}. "
