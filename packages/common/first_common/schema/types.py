...
class RuntimeScriptContext(TypedDict):
    """Values assigned by the pilot at replica launch time."""
    ...
    env: dict[str, str]
    """
    Merged template + deployment `env`. Also injected into the subprocess
    environment, so most templates do not need to reference it directly.
    """

    max_model_len: int | None
    """`max_model_len` of the parent Model; None when the Model omits it."""


class ScriptTemplateContext(TypedDict):
    """
    Variables made available to the `LaunchTemplate` serve and stop script
    templates when they are rendered.
    """

    runtime: RuntimeScriptContext
    parameters: dict[str, str | int | float | None]
    """Launch parameters resolved against the `LaunchTemplate` definitions."""
    quote: Callable[[str], str]
    """`shlex.quote`, also available as the `| quote` filter."""


def render_script(template: str, context: ScriptTemplateContext) -> str:
    """
    Render a script template with a strict context. The same function validates
    templates at apply time and renders them on the pilot, so anything that
    passes validation renders identically at launch.
    """
    env = Environment(undefined=StrictUndefined)
    env.filters["quote"] = shlex.quote
    try:
        return env.from_string(template).render(**context)
    except (TemplateSyntaxError, TemplateError, TypeError) as exc:
        raise ValueError(f"script template failed to render: {exc}") from exc


class ResolvedLaunchSpec(BaseModel):
    """
    A `LaunchSpec` merged with its `LaunchTemplate` by the gateway. This is what
    the pilot receives; it has already been validated and carries no validators.
    """
    ...
    pre_stop_script_template: str | None
    post_stop_script_template: str | None

    max_startup_sec: int
    max_unhealthy_sec: int | None
    pre_stop_timeout_sec: float
    post_stop_timeout_sec: float
