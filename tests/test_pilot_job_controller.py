...
async def test_scheduled_deletion_exiting_job_still_qdels_before_gone(
    db: async_sessionmaker[AsyncSession],
    ca_pair: tuple[str, str],
    adapter: FakeSchedulerAdapter,
) -> None:
    ...
    assert adapter.terminated == ["101.pbs"]
    job = await _get_job(db, uid)
...
async def test_exiting_job_still_consumes_concurrent_capacity(
    db: async_sessionmaker[AsyncSession],
    ca_pair: tuple[str, str],
    adapter: FakeSchedulerAdapter,
) -> None:
    async with db.begin() as sess:
        ...
        await _insert_pilot_job(
            sess, "still-allocated", scheduler_state=SchedulerJobState.exiting
        )
        uid = await _insert_pilot_job(
            sess, "waiting", scheduler_state=SchedulerJobState.pending_submit
        )
